...
int findmax(longint *a, longint *b)// swap to let the bigger at the front while return 1 if need no swap
{ 
    if (a->lenth > b->lenth)
        return 1;
...
{ 
    ...
    for (int i = 0; i < a->lenth; i++)
    {
        if (a->num[i] + x < b->num[i])// borrow from front
        { 
    ...
    while (c->num[highest] == '0' && highest != 0)
        c->lenth--, highest--; // delete the front zero
}
